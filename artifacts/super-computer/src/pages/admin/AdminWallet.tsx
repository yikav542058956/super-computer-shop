    ...
    <AdminLayout>
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                style={{ background: "#1F2937", paddingLeft: 36, paddingRight: 12, border: "1px solid #374151" }} />
            </div>
            {loading ? (
              <div className="space-y-2">{[...Array(5)].map((_, i) => <div key={i} className="h-16 rounded-xl animate-pulse" style={{ background: "#1F2937" }} />)}</div>
            ) : filtered.map(user => (
              <button key={user.id} onClick={() => setSelected(user)}
                ...
                style={{ border: "1px solid", background: selected?.id === user.id ? "rgba(34,197,94,0.06)" : "#111827" }}>
          ...
          <div className="lg:col-span-3">
            {!selected ? (
              ...
              <div className="space-y-4">
                {/* Balance Card */}
                <div className="p-5 rounded-2xl" style={{ background: "linear-gradient(135deg,#0f2318,#162B1E)", border: "1px solid rgba(34,197,94,0.2)" }}>
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">{selected.name || "User"}'s Wallet</p>
                      <p className="text-3xl font-black text-white mt-2">{formatINR(walletBalance)}</p>
                    </div>
                  ...
                  {txHistory.length === 0 ? (
                    ...
                    return (
                      <div key={tx.id} className="flex items-center gap-3 p-3 rounded-xl"
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-white truncate">{tx.note || tx.type}</p>
      ...
      {addOpen && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="w-full max-w-md rounded-3xl p-6 space-y-4" style={{ background: "#111827", border: "1px solid #374151" }}>
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-white">Wallet Transaction</h2>
              <button onClick={() => setAddOpen(false)}><X size={18} className="text-slate-400" /></button>
            </div>
            <p className="text-sm text-slate-400">User: <span className="text-white font-semibold">{selected.name}</span> · Balance: <span style={{ color: "#22C55E" }}>{formatINR(walletBalance)}</span></p>
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-2 block">Type</label>
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}
                className="w-full h-11 px-4 rounded-xl text-sm text-white outline-none"
                style={{ background: "#1F2937", border: "1px solid #374151" }}>
