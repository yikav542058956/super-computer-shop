      ...
      <div className="relative rounded-2xl overflow-hidden bg-[#F0F2F5]" style={{ height: "360px" }}>
        ...
        {imgList.length > 1 && (
          <>
            <button
              onClick={scrollPrev}
              className="absolute left-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-black/30 backdrop-blur-sm border border-white/20 text-white flex items-center justify-center hover:bg-black/50 transition-all"
            >
              <ChevronLeft className="h-5 w-5" />
            </button>
            <button
              onClick={scrollNext}
              className="absolute right-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-black/30 backdrop-blur-sm border border-white/20 text-white flex items-center justify-center hover:bg-black/50 transition-all"
            >
...
function ReviewCard({ review }: { review: any }) {
  return (
    <div className="bg-white/5 border border-white/8 rounded-2xl p-4 backdrop-blur-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <p className="font-semibold text-white text-sm">{review.userName || "Anonymous"}</p>
          ...
          <div className="flex gap-0.5 mt-1 mb-2">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star key={s} className={`h-3.5 w-3.5 ${s <= review.rating ? "fill-yellow-400 text-yellow-400" : "text-white/15 fill-transparent"}`} />
            ))}
          </div>
    ...
    return (
      <Layout>
        <div className="min-h-screen bg-[#0D1117] flex items-center justify-center flex-col gap-4">
          ...
          <p className="text-xl text-white font-bold">Product not found</p>
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-[#0D1117]">
        {/* Breadcrumb */}
        <div className="border-b border-white/5 bg-[#161B22]/50 backdrop-blur-sm">
        ...
        <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          {/* Main Product Card */}
          <div className="bg-[#161B22]/80 backdrop-blur-sm border border-white/8 rounded-3xl overflow-hidden mb-6 shadow-2xl">
            <div className="flex flex-col lg:flex-row">

              {/* ── Left: Image ── */}
              <div className="lg:w-[45%] p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-white/8">
                ...
                <div className="grid grid-cols-3 gap-2 mt-5">
                  {[
                    ...
                    <div key={label} className="flex flex-col items-center gap-1.5 bg-white/5 border border-white/8 rounded-xl py-3 px-2 text-center">
              ...
              <div className="lg:w-[55%] p-6 lg:p-8 flex flex-col">
                ...
                <h1 className="text-2xl lg:text-3xl font-black text-white leading-tight mb-4">{product.name}</h1>
                ...
                {product.specs && Object.keys(product.specs).length > 0 && (
                  <div className="grid grid-cols-2 gap-2.5 mb-6">
                    {Object.entries(product.specs).slice(0, 4).map(([key, val]: any) => (
                      <div key={key} className="flex items-center gap-2.5 bg-white/5 border border-white/8 rounded-xl p-3 group hover:border-green-500/30 transition-colors">
                  ...
                  <button
                    ...
                    className={`h-13 w-13 p-3.5 rounded-2xl border transition-all hover:scale-105 ${
                      isWishlisted(product.id)
                        ...
                        : "border-white/15 bg-white/5 text-slate-400 hover:border-white/30 hover:text-red-400"
                    }`}
                ...
                <div className="flex flex-wrap gap-2">
                  {["Cash on Delivery", "Easy EMI Available", "7-Day Returns"].map((tag) => (
                    <span key={tag} className="flex items-center gap-1 text-[11px] text-slate-400 bg-white/5 border border-white/8 px-2.5 py-1 rounded-full">
          ...
          <div className="bg-[#161B22]/80 backdrop-blur-sm border border-white/8 rounded-3xl overflow-hidden shadow-xl">
            <Tabs defaultValue="overview">
              <TabsList className="w-full justify-start rounded-none h-auto p-0 bg-transparent border-b border-white/8">
                {[
                  ...
                  <TabsTrigger
                    key={value}
                    value={value}
                    className="rounded-none px-6 py-4 text-sm font-semibold text-slate-400 border-b-2 border-transparent data-[state=active]:border-green-500 data-[state=active]:text-green-400 data-[state=active]:bg-transparent hover:text-white transition-colors"
                  >
                    {label}
                  </TabsTrigger>
                ))}
              ...
              <TabsContent value="overview" className="p-6 lg:p-8">
                <h3 className="text-xl font-black text-white mb-4">Product Description</h3>
                <p className="text-slate-400 leading-relaxed text-[15px]">{product.description || "No description available."}</p>
              </TabsContent>
                ...
                {product.specs && Object.keys(product.specs).length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                    {Object.entries(product.specs).map(([key, val]: any, idx) => (
                      <div
                        key={key}
                        className={`flex justify-between items-center py-3.5 px-4 gap-4 border-b border-white/5 ${
                          idx % 2 === 0 ? "bg-white/2" : ""
                        } hover:bg-white/5 transition-colors`}
                      >
                        <span className="text-slate-400 font-medium capitalize text-sm flex items-center gap-2">
                          <SpecsIcon label={key} />
                          {key}
                        </span>
                        <span className="text-white font-bold text-sm text-right pl-4">{val}</span>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              <TabsContent value="reviews" className="p-6 lg:p-8">
                {/* Rating summary */}
                <div className="flex items-center gap-6 mb-8 p-5 bg-white/5 border border-white/8 rounded-2xl">
                  <div className="text-center">
                    <p className="text-5xl font-black text-white">{avgRating}</p>
                    <div className="flex gap-0.5 justify-center mt-1">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} className={`h-4 w-4 ${s <= Math.round(Number(avgRating)) ? "fill-yellow-400 text-yellow-400" : "text-white/20 fill-transparent"}`} />
                      ))}
                    </div>
                ...
                {currentUser ? (
                  alreadyReviewed ? (
                    ...
                    <div className="mb-8 p-5 bg-white/5 border border-white/8 rounded-2xl">
                      ...
                      <textarea
                        ...
                        className="w-full bg-[#0D1117] border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-500 outline-none focus:border-green-500/50 transition-all resize-none"
                      />
                      <Button
                  ...
                  <div className="mb-6 p-4 bg-white/5 border border-white/8 rounded-2xl text-center">
