...
import {
  Plus, Upload, Loader2, X, ScanLine, CheckCircle, AlertCircle,
  Cpu, Camera, Sparkles,
} from "lucide-react";
...
interface Props {
  ...
  /** Called with the newly saved product after a successful add */
  onSaved?: (product: SavedProduct) => void;
  /** Pre-fill the product name (e.g. from offline sale search query) */
  initialName?: string;
}
...
/* ─── Component ───────────────────────────────────────── */
export default function AddProductDialog({ open, onOpenChange, onSaved, initialName }: Props) {
  const [categories, setCategories] = useState<any[]>([]);
  const [form, setForm] = useState({ ...EMPTY_FORM, specs: defaultSpecs() });
  const [uploading, setUploading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [generatingDesc, setGeneratingDesc] = useState(false);
  const [scanningBill, setScanningBill] = useState(false);
  const [fetchingSpecs, setFetchingSpecs] = useState(false);
  const [specsFetchedFor, setSpecsFetchedFor] = useState("");
  ...
  /* Reset form when dialog opens, pre-fill name if given */
  useEffect(() => {
    if (open) {
      setForm({ ...EMPTY_FORM, specs: defaultSpecs(), name: initialName || "" });
      setSpecsFetchedFor("");
      setAiExtracted(null);
  ...
  function applyExtracted(extracted: any) {
    ...
    setShowVerifyDialog(false);
    setAiExtracted(null);
    toast.success("Details filled — please verify and save.");
  }
  ...
  const handleSave = async () => {
    ...
    if (form.discountPrice && (isNaN(Number(form.discountPrice)) || Number(form.discountPrice) >= Number(form.price))) {
      toast.error("Sale Price must be less than MRP"); return;
    }
    ...
    try {
      ...
      const data: any = {
        ...
        isStudentPick: form.isStudentPick,
        isGamingDeal: form.isGamingDeal,
        status: form.status,
        rating: 0,
        reviewsCount: 0,
        createdAt: Date.now(),
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="rounded-xl border-2 border-dashed p-4 flex items-center justify-between gap-3"
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => { if (billScanRef.current) { billScanRef.current.setAttribute("capture", "environment"); billScanRef.current.click(); } }}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving || uploading}>
              {saving ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Saving...</> : "Add Product"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
